...
void loop(){
  ...
  //y range: -90-90
  //z range: -180-180
  int val_X = map(euler.y(), 0, 360, -25, 25);  //remap the limits from -10 and 90 to 0 and 5.
  int val_Y = map(euler.y(), -90, 90, -25, 25);  //remap the limits from -10 and 90 to 0 and 5.
  int val_Z = map(euler.y(), -180, 180, -25, 25);  //remap the limits from -10 and 90 to 0 and 5.
  val = val*10;
  rotateZ(val);
  rotateX(val);
}
